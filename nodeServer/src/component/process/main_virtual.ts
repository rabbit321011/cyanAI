...
export let main_status:total_status|null = null;
...
async function processImageSummaries(expireTime: string, maxRecentImages: number, summary_turns: number): Promise<void> {
    if (!main_status) return;
    
    try {
        compare(expireTime, '0s');
    } catch (e) {
        console.error('[processImageSummaries] expireTime 格式错误:', e);
        return;
    }//检测expireTime格式是否正确
    
    const currentTime = now();
    
    // 从后往前统计图片数量和轮次，标记哪些图片需要总结
    let recentImageCount = 0;
    let rethink_count = 0;
    const messageImageStatus: { index: number; isRecent: boolean; isOverTurns: boolean; imageCount: number }[] = [];
    
    for (let i = main_status.context.length - 1; i >= 0; i--) {
        const msg = main_status.context[i];
        
        // 检测新 rethink 开始
        if (msg.role_type === 'user' && msg.role !== 'system') {
            rethink_count++;
        }
        ...
        if (msg.inline && msg.inline.length > 0) {
            ...
            const isOverTurns = rethink_count > summary_turns;
            messageImageStatus.unshift({ index: i, isRecent, isOverTurns, imageCount });
    ...
    for (const status of messageImageStatus) {
        ...
        // 如果不是最近的图片、已过期或超过轮次，需要总结
        if (!status.isRecent || isExpired || status.isOverTurns) {
            // 总结每张图片
            ...
            const imageDescription = summaries.map((s, idx) => {
                const prefix = summaries.length === 1 ? '图片触发总结机制，原始图片的内容是：' : `图片${idx + 1}触发总结机制，原始图片的内容是：`;
...
let now_policy:policy_group = {
    ...
    temperature: 0.7,
    monologue: true,
    monologue_summary_turn: 0,
    monologue_summary_turn_precision: 0,
    summary_turn: 0,//这是图片啥的的保留轮次
    summary_turn_time : "10min",
    summary_turn_precision: 0,
    image_max: 10,
    video_max: 10,
    audio_max: 10,
    multimedia_max: 10,
    tools_files: ["main_virtual\\main.json"],
    talk_commands: [],
    talk_description_file: "",
    direct_tools: ["default_api:any-example","default_api:main-virtual-speak","default_api:send_QQ_message"],
    auto_rag_topk: 0,
    auto_reranker_mode: "bge",
    auto_emerge_rate: 0,
    auto_emerge_frequency: 0,
    workspace: false,
    pull_info: false,
    step_progress: false,
    change_policy_group: [],
    system_prompt_generator: "default.ts",
    max_events: 100,
    events_threshold: 0,
