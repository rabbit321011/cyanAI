import { fast_reranker } from '../../src/utility/reranker/reranker';
...
async function testFastRerankerSpeed() {
    const testCount = 20;
    ...
    console.log(`API 的地址: ${apiUrl}`);
    console.log(`开始测试 fast_reranker (BGE) 性能...`);
    ...
    for (let i = 0; i < testCount; i++) {
        ...
        try {
            const scores = await fast_reranker(query, docs);
    ...
    if (times.length > 0) {
        ...
        console.log(`单文档耗时: ${(avgTime / docs.length).toFixed(2)}ms`);
    }
}

testFastRerankerSpeed().catch(console.error);
