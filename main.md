...
<system>:
    ...
    0. 对话场景
        ...
        在对话的开头有 "^userName:"，这表示这句话是 userName 说的（userName 不固定，且一次回答可能有多个）。如果userName为${agent_name},则表示这条内容是你说的
    ...
    2. 文件表示
        如果你发送 %[xxx.xxx] 会被转义，xxx.xxx 文件会解析后呈现。但是注意，%[]内需是完整路径
    ...
    一般来说尽量只回复一句或几句就行，模仿日常对话的语气。
    你的回答应该让对方感到舒适，具体来说就是在对方提问时认真回答，谈心的时候认真倾听，说再见时可以适当简短的道别，该短回复的时候就短回复（如能说“那我就在这里默默为你加油！”的场景就不要长篇大论），该自由散漫聊天的时候就可以适当放开和多说一些。
...
<system prompt>
    主要提示词(数据类型-string)
    人设参考(数据类型-string，这玩意是插入上面的主要提示词的)
    粗略工具调用(数据类型-)//已废弃
    近期事件记录(数据类型-事件数组)
    工作区(数据类型-文献复合体)
    对象与关系区(数据类型-computed-关系对象复合体)
    拉取的信息区(数据类型-文件列表(显示了序号))
    计划步进(数据类型-计划列表)
<context>
    user: 时间戳 上一个话题开始点
...
```
# 拉取到的内容
%[xxx.xxx] - 该文件的简介，当然，方括号里面的是完整路径
%[xxx.xx2] - 文件二的简介
```

这个拉取到的信息区的存在意义是给agent审核拉取到的信息，以及存储或者选择放弃当前拉取到的信息，又或者是提取其部分存储

实际上，tools的返回是会存在于聊天记录里的，但是本区记载的是本次对话获得的易失性的文件，tools的聊天记录里只有一个%[xxx.xxx],当然还是完整路径
...
模型老是出幻觉输出时间，该转义器的目的是去掉输出的时间

模型可能会神经错乱输出多个时间，所以该转义器会去除全部的时间

时间如：^cyanAI:20260224_163737:^cyanAI:20260224_163748:^cyanAI:20260224_163750:^cyanAI:20260224_163755:^cyanAI:20260224_163800:^cyanAI:20260224_163805:然后这里才是真的字符串
